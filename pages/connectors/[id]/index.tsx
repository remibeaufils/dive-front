...
const ConnectorPage: NextPage<Props> = ({ connector }) => {
  ...
  return (
    <>
      <Row gutter={16}>
        ...
        {adAccounts &&
          adAccounts.map(({ business_name, name, account_id, insights }, index: number) => (
        ...
        {configured && (
          <Col span={24}>
            <Card className="card" style={{ textAlign: 'center' }}>
              <Button type="primary" onClick={removeConnection}>
                Remove Connection
              </Button>
            </Card>
          </Col>
...
export const getServerSideProps = withAuth(
  async ({ ctx, user }: { ctx: GetServerSidePropsContext; user: any }) => {
    const data = await fetch(`/connectors/${ctx.query.id}`, null, ctx)
